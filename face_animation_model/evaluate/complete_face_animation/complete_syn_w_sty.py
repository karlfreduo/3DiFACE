...
class test_w_sty_head():
    ...
    def get_edit_batch(self, args, pred_file):
        ...
        if True:
            pose = scipy.ndimage.filters.gaussian_filter1d(pose, sigma=1, axis=0)
    ...
    def run_edit_external_audio(self, args):
        ...
        for edit_file in to_run:
            ...
            for i in range(exp_cfg.num_of_samples):
                ...
                if True:
                    pose_pred_normed_space = scipy.ndimage.filters.gaussian_filter1d(pose_pred_normed_space[0].cpu().numpy(), sigma=0.9, axis=0)
                    pose_pred_normed_space = torch.from_numpy(pose_pred_normed_space).float().unsqueeze(0).to(device=audio_feat.device)
